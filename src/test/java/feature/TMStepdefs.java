...
public class TMStepdefs {
	...
	public void dieTMMitDemNamenWirdPersistiertUndWiederGeladen(final String nameDerTM) {
		...
		final TuringMaschinenBuilder geladeneTuringMaschine = (TuringMaschinenBuilder) Persistierer.getInstance()
				.vonXML(xml);
	...
	public void dieTMMitDemNamenHatBeiFolgenderEingabeDieFolgendeAusgabeAufBand(final String nameDerTM,
		...
		final TuringMaschine tm = this.getTM(nameDerTM).build();
		/*
		 * for (EingabeAusgabeDAO eingabe : eingabenAusgaben) { final List<String>
		 * eingaben = new ArrayList<>(this.splitSemikolon(eingabe.getEingabe()));
		 * System.out.println(eingabe.getEingabe()); final Set<Konfiguration> simuliere
		 * = tm.simuliere(eingaben); System.out.println(simuliere); }
		 */
	...
	private boolean zurEingabeGibtEsPassendeAusgabe(final TuringMaschine tm, final EingabeAusgabeDAO eingabeAusgabe,
		...
		return tm.simuliere(eingabeAusgabe.getEingabe().split(SEMIKOLOn)).stream()
